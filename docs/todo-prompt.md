...
Prévois les statuts `BACKLOG`, `TODO`, `IN_PROGRESS`, `TO_TEST`, `DONE` et `CANCELLED`, avec les libellés français Backlog, À faire, En cours, À tester, Terminé et Annulé. Centralise la configuration des statuts pour pouvoir les adapter plus tard.
...
Affiche les tâches du projet sélectionné en vue liste par défaut et en vue kanban, avec un toggle dont le choix reste mémorisé localement. Mémorise aussi le dernier projet sélectionné avec un fallback si le projet n'existe plus. Permets de créer une tâche dans le projet actif depuis une colonne ou section, modifier titre, description et statut, supprimer une tâche, et avancer rapidement une tâche vers le statut logique suivant.
